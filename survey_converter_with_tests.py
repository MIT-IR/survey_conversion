...
def get_survey_metadata(file_name, clean_dims):
    ...
    #creates an object that returns the variables in the file
    sdict = spssaux.VariableDict()
    dimension_labels = {}
    var_list = sdict.expand("all")
    for i in range(len(var_list)):
        var = var_list[i]
        var_index = sdict.VariableIndex(var)
        # Currently doesn't get stems, if they were to be
        # included, they would be extracted here
        stem = ""
        question_label = str(spss.GetVariableLabel(var_index).encode('ascii','ignore'))
        vallabs = sdict[var_index].ValueLabels
        if vallabs is None:
...
@nose.with_setup(teardown=teardown_func)
def metadata_value_label_output_file_complete_test():
    """Test the contents of the file that the get_metadata() fn creates"""
    test_filename = "var_label_val_label_testing"
    null = get_survey_metadata(test_filename,["var1"])
    f = open("".join((test_filename,"_values_input.txt")))
    correct_lines = [
                     "var1|1|ONE|variable one|\n",
                     "var1|2|TWO|variable one|\n",
                     "var1|3|THREE|variable one|\n",
                     "var2|4|FOUR|variable two|\n",
                     "var2|5|FIVE|variable two|\n",
                     "var3|a|THE LETTER A|variable three|\n",
                     "var3|b|THE LETTER B|variable three|\n",
                     "var4|None|None|variable four|\n"
    ]
